...
class IdentityManager:
    ...
    # ---------- app 校验 ----------
    def ensure_app_active(self, app_id: str) -> None:
        """
        Identity 只负责校验 app 是否可用（active）。
        “是否允许代用户操作数据”的权限校验应由 API 层完成。
        """
    ...
    def generate_memory_key(wallet_id: str, app_id: str, session_id: str) -> str:
        """
        注意：这里的 wallet_id 语义是“数据归属钱包（业务用户）”，不是操作方钱包。
        """
    ...
    def resolve_identity(
        ...
        *,
        ...
        """
        self.ensure_app_active(app_id)

        data_wallet_id = normalize_wallet_id(data_wallet_id) or normalize_wallet_id(wallet_id)
        if not data_wallet_id:
            raise ValueError("data_wallet_id is empty")

        # 查找是否已存在会话
        row = self.session_store.get_by_triplet(data_wallet_id, app_id, session_id)
        if row:
            memory_key = row["memory_key"]
        else:
            # 创建新的 memory_key
            memory_key = self.generate_memory_key(data_wallet_id, app_id, session_id)
            self.session_store.upsert(memory_key, data_wallet_id, app_id, session_id)
        ...
        if self.private_db_store:
            ...
            self.private_db_store.bind_session(
